...
class Self_Consistency_re:
    def __init__(self, llm_model, model_name, few_shot_n, test_n, q_src_yn, ver, p_ver, sf_num, temperature, excel_ver, i):  
        # init variables
        ...
        self.logger.info(f'param for sample self consistency : {llm_model} | {model_name} | {few_shot_n} | {q_src_yn} | {p_ver} | {sf_num} | {temperature} | {excel_ver}' )
        
        self.save_dir = f'{conf.DATA_PATH}{conf.ANNO_RESULT}/{model_name}'
        if not os.path.exists(self.save_dir):
            os.makedirs(self.save_dir)  
        self.save_file = f'sc_{llm_model}_result_{few_shot_n}_{self.test_n}_{q_src_yn}_{self.version}_{self.p_ver}_{self.sf_num}_{self.temperature}_{self.excel_ver}_{self.loop_i}'
    ...
    def calc_acc_for_l(self, llm_model, few_shot_n, q_src_yn):           
        ...
        result_df.to_csv(f'{self.save_dir}/{self.save_file}.csv')
        file_io.save_json(conf.VLLM_CONF[llm_model], f'{self.save_dir}/{self.save_file}_llm_config.json')
        # result_df.to_csv(f'{self.save_dir}/sc_{llm_model}_result_{few_shot_n}_{self.test_n}_{q_src_yn}_{self.version}_{self.p_ver}_{self.sf_num}_{self.temperature}_{self.excel_ver}_{self.loop_i}.csv')
